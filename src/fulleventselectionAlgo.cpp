		...
		if(PU_ScaleUp == true){
			...
			if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        ...
                        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                                ...
                                _cutflowstring = "CutFlowReport_" + process + "_ME_Up_" + year + "_NPL_ttbarCR_Blinded.txt";
                        }
                        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
...
if(year == "2016"){
	...
	else if(process == "SingleTop_tWZ_tWll"){input_files = {"/data/disk2/nanoAOD_2016/tWZ_tWLL_NanoAODv5/*"};} 
	else if(process == "ttbar_2l2nu"){input_files = {"/data/disk2/nanoAOD_2016/ttbar_2l2nu_NanoAODv5/*"};}
	else if(process == "ttbar_madgraph_NanoAODv5"){input_files = {"/data/disk2/nanoAOD_2016/ttbar_madgraph_NanoAODv5/*"};}
	...
	else if(process == "MC_triggerSF_ttbar"){input_files = {"/data/disk2/nanoAOD_2016/ttbar_2l2nu/*.root"};}
	else if(process == "MC_triggerSF_ZPlusJets"){input_files = {"/data/disk2/nanoAOD_2016/DYJetsToLL_NanoAODv5/*", "/data/disk2/nanoAOD_2016/DYJetsToLL_ext_NanoAODv5/*", "/data/disk2/nanoAOD_2016/DYJetsToLL_M10to50/*"};}
	...
	else if(process == "ttbar_2l2nu"){input_files = {"/data/disk1/nanoAOD_2018/ttbar_2l2nu_NanoAODv5/*"};}
	else if(process == "ttbar_madgraph_NanoAODv5"){input_files = {"/data/disk1/nanoAOD_2018/ttbar_madgraph_NanoAODv5/*"};}
	...
	else if(process == "MC_triggerSF_ttbar"){input_files = {"/data/disk1/nanoAOD_2018/ttbar_2l2nu/*.root"};}
...
auto NormalisationFactorFunction{[&process, &year](){
  ...
  std::vector<std::string> ProcessStrings = {" ", "tZq", "ZPlusJets_M50_aMCatNLO", "ZPlusJets_M50_aMCatNLO_ext", "ZPlusJets_M50_Madgraph", "ZPlusJets_M50_Madgraph_ext",
				    ...
				    "ttbar_TTToSemileptonic", "ttbar_atMCaNLO", "SingleTop_tchannel_top", "SingleTop_tchannel_top_ScaleUp", 
...
auto DummyBool{[](const bool& HLT_ZeroBias_part7){

  return HLT_ZeroBias_part7 > 0;
...
process != "data_SingleMuonRunH" 
  ...
  auto d_ee_selection_defines = d_EventCleaning.Define("DummyBool", DummyBool, {"HLT_ZeroBias_part7"})
...
auto d_mumu_selection_defines = d_EventCleaning.Define("DummyBool", DummyBool, {"HLT_ZeroBias_part7"})
					       .Define("PU", PU_function, {"PV_npvs"})
     ...
     auto d_emu_selection_defines = d_EventCleaning.Define("DummyBool", DummyBool, {"HLT_ZeroBias_part7"})
						   .Define("PU", PU_function, {"PV_npvs"})
...
if(blinding == false){
	...
	h_lightjets_ee = (TH2F*)h_charm_ee_num->Clone();
	h_lightjets_ee->Divide(h_charm_ee_denom.GetPtr());
...
auto EffNonBTaggedFunction_ee{[&h_charm_ee_num, &h_charm_ee_denom, &h_lightjets_ee_num, &h_lightjets_ee_denom, &h_gluon_ee_num, &h_gluon_ee_denom, &NBins](const floats& DummyColumn, const floats& pts, const floats& etas){
  ...
  for(int i = 0; i < pts.size(); i++){
	...
	//charm
        int PtNum_charm = h_charm_ee_num->GetXaxis()->FindBin(pts.at(i));
        int EtaNum_charm = h_charm_ee_num->GetYaxis()->FindBin(etas.at(i));

        int PtDenom_charm = h_charm_ee_denom->GetXaxis()->FindBin(pts.at(i));
        int EtaDenom_charm = h_charm_ee_denom->GetYaxis()->FindBin(etas.at(i));

        float Numerator_charm = h_charm_ee_num->GetBinContent(PtNum_charm, EtaNum_charm);
        float Denominator_charm = h_charm_ee_denom->GetBinContent(PtDenom_charm, EtaDenom_charm);

        float eff_charm = Numerator_charm / Denominator_charm;

	//light jets
	int PtNum_lightjets = h_lightjets_ee_num->GetXaxis()->FindBin(pts.at(i));
        int EtaNum_lightjets = h_lightjets_ee_num->GetYaxis()->FindBin(etas.at(i));

        int PtDenom_lightjets = h_lightjets_ee_denom->GetXaxis()->FindBin(pts.at(i));
        int EtaDenom_lightjets = h_lightjets_ee_denom->GetYaxis()->FindBin(etas.at(i));
  
        float Numerator_lightjets = h_lightjets_ee_num->GetBinContent(PtNum_lightjets, EtaNum_lightjets);
        float Denominator_lightjets = h_lightjets_ee_denom->GetBinContent(PtDenom_lightjets, EtaDenom_lightjets);

        float eff_lightjets = Numerator_lightjets / Denominator_lightjets;

	//gluon
	int PtNum_gluon = h_gluon_ee_num->GetXaxis()->FindBin(pts.at(i));
        int EtaNum_gluon = h_gluon_ee_num->GetYaxis()->FindBin(etas.at(i));

        int PtDenom_gluon = h_gluon_ee_denom->GetXaxis()->FindBin(pts.at(i));
        int EtaDenom_gluon = h_gluon_ee_denom->GetYaxis()->FindBin(etas.at(i));
  
        float Numerator_gluon = h_gluon_ee_num->GetBinContent(PtNum_gluon, EtaNum_gluon);
        float Denominator_gluon = h_gluon_ee_denom->GetBinContent(PtDenom_gluon, EtaDenom_gluon);

        float eff_gluon = Numerator_gluon / Denominator_gluon;
...
auto EffNonBTaggedFunction_mumu{[&h_charm_mumu_num, &h_charm_mumu_denom, &h_lightjets_mumu_num, &h_lightjets_mumu_denom, &h_gluon_mumu_num, &h_gluon_mumu_denom, &NBins](const floats& DummyColumn, const floats& pts, const floats& etas){
  ...
  for(int i = 0; i < pts.size(); i++){
	...
	//charm
        int PtNum_charm = h_charm_mumu_num->GetXaxis()->FindBin(pts.at(i));
        int EtaNum_charm = h_charm_mumu_num->GetYaxis()->FindBin(etas.at(i));

        int PtDenom_charm = h_charm_mumu_denom->GetXaxis()->FindBin(pts.at(i));
        int EtaDenom_charm = h_charm_mumu_denom->GetYaxis()->FindBin(etas.at(i));

        float Numerator_charm = h_charm_mumu_num->GetBinContent(PtNum_charm, EtaNum_charm);
        float Denominator_charm = h_charm_mumu_denom->GetBinContent(PtDenom_charm, EtaDenom_charm);

        float eff_charm = Numerator_charm / Denominator_charm;

	//light jets
	int PtNum_lightjets = h_lightjets_mumu_num->GetXaxis()->FindBin(pts.at(i));
        int EtaNum_lightjets = h_lightjets_mumu_num->GetYaxis()->FindBin(etas.at(i));

        int PtDenom_lightjets = h_lightjets_mumu_denom->GetXaxis()->FindBin(pts.at(i));
        int EtaDenom_lightjets = h_lightjets_mumu_denom->GetYaxis()->FindBin(etas.at(i));
  
        float Numerator_lightjets = h_lightjets_mumu_num->GetBinContent(PtNum_lightjets, EtaNum_lightjets);
        float Denominator_lightjets = h_lightjets_mumu_denom->GetBinContent(PtDenom_lightjets, EtaDenom_lightjets);

        float eff_lightjets = Numerator_lightjets / Denominator_lightjets;

	//gluon
	int PtNum_gluon = h_gluon_mumu_num->GetXaxis()->FindBin(pts.at(i));
        int EtaNum_gluon = h_gluon_mumu_num->GetYaxis()->FindBin(etas.at(i));

        int PtDenom_gluon = h_gluon_mumu_denom->GetXaxis()->FindBin(pts.at(i));
        int EtaDenom_gluon = h_gluon_mumu_denom->GetYaxis()->FindBin(etas.at(i));
  
        float Numerator_gluon = h_gluon_mumu_num->GetBinContent(PtNum_gluon, EtaNum_gluon);
        float Denominator_gluon = h_gluon_mumu_denom->GetBinContent(PtDenom_gluon, EtaDenom_gluon);
...
if(blinding == true){
	...
	for(int i = FirstChi2Entry_ee; i < MiddleBin_ee; i++){
		for(int j = LastChi2Entry_ee; j > MiddleBin_ee-1; j++){
			...
			auto Chi2Cut_ee{[&h_chi2_ee, &i, &j, &Min_ee, &Max_ee](const float& Chi2){
	...
	for(int i = FirstChi2Entry_mumu; i < MiddleBin_mumu; i++){
			...
			auto Chi2Cut_mumu{[&h_chi2_mumu, &i, &j, &Min_mumu, &Max_mumu](const float& Chi2){
	...
	auto chi2_filter_mumu{[&MaxChi2_mumu, &MinChi2_mumu](const float& chi2_mumu){
...
std::string tZq_WAndTop_Filename; 
  ...
  if(PU_ScaleUp == true){
	...
	if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_PU_ScaleUp" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_PU_ScaleUp" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_PU_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_PU_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_PU_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_PU_ScaleUp" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_PU_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_PU_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_PU_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_PU_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_PU_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_PU_ScaleDown" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_BTag_ScaleUp" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_BTag_ScaleUp" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_BTag_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_BTag_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_BTag_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_BTag_ScaleUp" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_BTag_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_BTag_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_BTag_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_BTag_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_BTag_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_BTag_ScaleDown" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_JetSmearing_ScaleUp" + year + ".root";
        }       
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_JetSmearing_ScaleUp" + year + ".root";
        }       
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_JetSmearing_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_JetSmearing_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_JetSmearing_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_JetSmearing_ScaleUp" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_JetSmearing_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_JetSmearing_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_JetSmearing_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_JetSmearing_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_JetSmearing_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_JetSmearing_ScaleDown" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_JetResolution_ScaleUp" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_JetResolution_ScaleUp" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_JetResolution_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_JetResolution_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_JetResolution_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_JetResolution_ScaleUp" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_JetResolution_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_JetResolution_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_JetResolution_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_JetResolution_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_JetResolution_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_JetResolution_ScaleDown" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_LeptonEfficiencies_ScaleUp" + year + ".root";
        }       
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_LeptonEfficiencies_ScaleUp" + year + ".root";
        }       
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_LeptonEfficiencies_ScaleUp" + year + ".root";
        }       
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_LeptonEfficiencies_ScaleUp" + year + ".root";
        }       
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_LeptonEfficiencies_ScaleUp" + year + ".root";
        }       
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_LeptonEfficiencies_ScaleUp" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_LeptonEfficiencies_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_LeptonEfficiencies_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_LeptonEfficiencies_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_LeptonEfficiencies_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_LeptonEfficiencies_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_LeptonEfficiencies_ScaleDown" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_PDF_ScaleUp" + year + ".root";
        }       
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_PDF_ScaleUp" + year + ".root";
        }       
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_PDF_ScaleUp" + year + ".root";
        }       
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_PDF_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_LeptonEfficiencies_ScaleUp" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_PDF_ScaleUp" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_PDF_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_PDF_ScaleDown" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_PDF_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_PDF_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_PDF_ScaleDown" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_PDF_ScaleDown" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ME_Up" + year + ".root";
        }       
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_ME_Up" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_ME_Up" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_ME_Up" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_ME_Up" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ME_Up" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ME_Down" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_ME_Down" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_ME_Down" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_ME_Down" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_ME_Down" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ME_Down" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_isr_up" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_isr_up" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_isr_up" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_isr_up" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_isr_up" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_isr_up" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_isr_down" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_isr_down" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_isr_down" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_isr_down" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_isr_down" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_isr_down" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_fsr_up" + year + ".root";
        }       
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_fsr_up" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_fsr_up" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_fsr_up" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_fsr_up" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_fsr_up" + year + ".root";}
        ...
        if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_fsr_down" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ZPlusJetsCR_fsr_down" + year + ".root";
        }
        else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_ttbarCR_fsr_down" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ZPlusJetsCR_fsr_down" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_NPL_ttbarCR_fsr_down" + year + ".root";
        }
        else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        else{tZq_WAndTop_Filename = "tZq_AfterFullSelection_GaussianFit_fsr_down" + year + ".root";}
...
if(process == "tZq"){
	...
	if(PU_ScaleUp == true){
        	...
        	if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                	filenamestring_variable = "Resolution_PU_ScaleUp_" + year + "_NPL.txt";
        	}
        	else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                	filenamestring_variable = "Resolution_PU_ScaleUp_" + year + "_ZPlusJetsCR.txt";
        	}
        	else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                	filenamestring_variable = "Resolution_PU_ScaleUp_" + year + "_ttbarCR.txt";
        	}
        	else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                	filenamestring_variable = "Resolution_PU_ScaleUp_" + year + "_NPL_ZPlusJetsCR.txt";
        	}
        	else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                	filenamestring_variable = "Resolution_PU_ScaleUp_" + year + "_NPL_ttbarCR.txt";
        	}
        	else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
        	else{filenamestring_variable = "Resolution_PU_ScaleUp_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_PU_ScaleDown_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_PU_ScaleDown_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_PU_ScaleDown_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_PU_ScaleDown_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_PU_ScaleDown_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_PU_ScaleDown_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_BTag_ScaleUp_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_BTag_ScaleUp_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_BTag_ScaleUp_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_BTag_ScaleUp_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_BTag_ScaleUp_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_BTag_ScaleUp_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_BTag_ScaleDown_" + year + "_NPL.txt";
                }       
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_BTag_ScaleDown_" + year + "_ZPlusJetsCR.txt";
                }       
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_BTag_ScaleDown_" + year + "_ttbarCR.txt";
                }       
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_BTag_ScaleDown_" + year + "_NPL_ZPlusJetsCR.txt";
                }       
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_BTag_ScaleDown_" + year + "_NPL_ttbarCR.txt";
                }       
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_BTag_ScaleDown_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleUp_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleUp_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleUp_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleUp_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleUp_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_JetSmearing_ScaleUp_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleDown_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleDown_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleDown_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleDown_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_JetSmearing_ScaleDown_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_JetSmearing_ScaleDown_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetResolution_ScaleUp_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetResolution_ScaleUp_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_JetResolution_ScaleUp_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetResolution_ScaleUp_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_JetResolution_ScaleUp_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_JetResolution_ScaleUp_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetResolution_ScaleDown_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetResolution_ScaleDown_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_JetResolution_ScaleDown_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_JetResolution_ScaleDown_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_JetResolution_ScaleDown_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_JetResolution_ScaleDown_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleUp_" + year + "_NPL.txt";
                }       
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleUp_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleUp_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleUp_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleUp_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleUp_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleDown_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleDown_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleDown_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleDown_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleDown_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_LeptonEfficiencies_ScaleDown_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_PDF_ScaleUp_" + year + "_NPL.txt";
                }       
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_PDF_ScaleUp_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_PDF_ScaleUp_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_PDF_ScaleUp_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_PDF_ScaleUp_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_PDF_ScaleUp_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_PDF_ScaleDown_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_PDF_ScaleDown_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_PDF_ScaleDown_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_PDF_ScaleDown_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_PDF_ScaleDown_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_PDF_ScaleDown_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_ME_Up_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_ME_Up_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_ME_Up_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_ME_Up_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_ME_Up_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_ME_Up_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_ME_Down_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_ME_Down_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_ME_Down_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_ME_Down_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_ME_Down_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_ME_Down_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_isr_up_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_isr_up_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_isr_up_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_isr_up_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_isr_up_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_isr_up_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_isr_down_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_isr_down_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_isr_down_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_isr_down_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_isr_down_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_isr_down_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_fsr_up_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_fsr_up_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_fsr_up_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_fsr_up_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_fsr_up_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_fsr_up_" + year + ".txt";}
                ...
                if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
                        filenamestring_variable = "Resolution_fsr_down_" + year + "_NPL.txt";
                }
                else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_fsr_down_" + year + "_ZPlusJetsCR.txt";
                }
                else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_fsr_down_" + year + "_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
                        filenamestring_variable = "Resolution_fsr_down_" + year + "_NPL_ZPlusJetsCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
                        filenamestring_variable = "Resolution_fsr_down_" + year + "_NPL_ttbarCR.txt";
                }
                else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
                else{filenamestring_variable = "Resolution_fsr_down_" + year + ".txt";}
		...
		if(NPL == true && ZPlusJetsCR == false & ttbarCR == false){
			filenamestring_variable = "Resolution_" + year + "_NPL.txt";
		}
		else if(NPL == false && ZPlusJetsCR == true & ttbarCR == false){
			filenamestring_variable = "Resolution_" + year + "_ZPlusJetsCR.txt";
		}
		else if(NPL == false && ZPlusJetsCR == false & ttbarCR == true){
			filenamestring_variable = "Resolution_" + year + "_ttbarCR.txt";
		}
		else if(NPL == true && ZPlusJetsCR == true & ttbarCR == false){
			filenamestring_variable = "Resolution_" + year + "_NPL_ZPlusJetsCR.txt";
		}
		else if(NPL == true && ZPlusJetsCR == false & ttbarCR == true){
			filenamestring_variable = "Resolution_" + year + "_NPL_ttbarCR.txt";	
		}
		else if(NPL == true && ZPlusJetsCR == true & ttbarCR == true){std::cout << "Error: NPL, ZPlusJetsCR and ttbarCR cannot all be true." << std::endl;}
		else{filenamestring_variable = "Resolution_" + year + ".txt";}
...
if(blinding == true){
	...
	//double MaxChi2 = 30; //need to actually calculate
	//double MinChi2 = 5; //need to actually calculate
...
*/
  ...
  for(int i = 0; i < year.size(); i+){DirectoryCreator(year, blinding, NPL);}
