...
for i in ${!ProcessNamesArray[@]}; do
	...
	k=$(($ChannelInt - 1))
        ...
        echo './bin/fulleventselectionMain.exe --mc 0 -y '$Year' -p '$j' --npl 0 --sr '$SRInt' --sbr '$SBRInt' --zjcr '$zjcrInt' --ttcr '$ttcrInt' --sys 0 --channel '$ChannelInt' --dcc 0'

        tmux new -d -s $tmux_string './bin/fulleventselectionMain.exe --mc 0 -y '$Year' -p '$j' --npl 0 --sr '$SRInt' --sbr '$SBRInt' --zjcr '$zjcrInt' --ttcr '$ttcrInt' --sys '$SystematicInt' --channel '$ChannelInt' --dcc 0; sleep 3600'
