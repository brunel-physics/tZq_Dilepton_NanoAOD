...
void plotter(variable var, string year, string channel, string systematic, string region, bool include_signal, bool include_data, bool normalise)
{
	...
	if(systematic == "Nominal"){
        	file = "Results_BackUp/" + year + "/" + channel + "/" + region + "/Nominal/" + "histofile_" + proc.name + "_Nominal.root";
		treename = "Ttree_" + proc.name;
	}
	else{
		file = "Results_BackUp/" + year + "/" + channel + "/" + region + "/Systematics/" + "histofile_" + proc.name + "_" + systematic + ".root";
                treename = "Ttree_" + proc.name + "__" + systematic;
	}
 

	std::cout << "file = " << file << ". TTree = " << treename << std::endl;

        RDataFrame data_frame(treename.c_str(), file.c_str());
    ...
    std::string normalise_string;

    // Normalise if requested
    if(normalise)
    {
  	normalise_string = "_normalised";
    ...
    else{normalise_string = "_notnormalised";}
    ...
    TPad *pad2 = new TPad("pad2", "pad2", 0, 0, 1, 0.3275);
    pad2->SetTopMargin(0);
    pad2->SetFillColor(0);
    ...
    TH1D * rp = (TH1D*)(hist["data"]->Clone());
    rp->Divide((TH1D*)bg_stack->GetStack()->Last());
    rp->SetStats(false);
    ...
    rp->GetXaxis()->SetTitle(var.title.c_str());
    rp->SetMinimum(0.5);
    rp->SetMaximum(1.5);
    rp->Draw(); 
    ...
    string outfile = "Plots/" + var.name + norm_str + "_" + channel + "_" + systematic + "_" + region + "_" + year + "_" + normalise_string + ".pdf";
    c1->Update();
            ...
            for(const auto & systematic: systematics)
                for(const auto & region: regions)
                    for(const auto & var: variables)
                        plotter(var, year, channel, systematic, region, true, true, true); //var, year, channel, systematic, region, include_signal, include_data, normalise
