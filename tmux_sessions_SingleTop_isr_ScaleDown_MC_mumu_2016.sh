...
for i in ${!ProcessNamesArray[@]}; do
	...
	tmux_string="${ProcessNamesArray[i]}${SystematicNamesArray[18]}${ChannelArray[1]}_${YearArray[0]}"

	j=$(($i + 38))

	echo './bin/fulleventselectionMain.exe --mc 1 -y 2016 -p '$j' --npl 0 --sr 1 --sbr 0 --zjcr 0 --ttcr 0 --sys 18 --channel 2 --dcc 0'

	tmux new -d -s $tmux_string 'source /cvmfs/sft.cern.ch/lcg/views/LCG_96/x86_64-slc6-gcc8-opt/setup.sh; make clean; make; ./bin/fulleventselectionMain.exe --mc 1 -y 2016 -p '$j' --npl 0 --sr 1 --sbr 0 --zjcr 0 --ttcr 0 --sys 18 --channel 2 --dcc 0; sleep 86400'
...
for i in ${!ProcessNamesArray2[@]}; do
        ...
        tmux_string="${ProcessNamesArray2[i]}${SystematicNamesArray[18]}${ChannelArray[1]}_${YearArray[0]}"
...
for i in ${!ProcessNamesArray3[@]}; do
        ...
        tmux_string="${ProcessNamesArray3[i]}${SystematicNamesArray[18]}${ChannelArray[1]}_${YearArray[0]}"
...
echo "The tmux sessions for single top processes in the SR (mumu channel for 2016, isr scale down) are running."
